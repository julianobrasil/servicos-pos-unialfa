...
export class AlocacaoDeTarefasService {
  ...
  findAllUsersFromTarefaId(tarefaId: number): Observable<UsuarioEntityTo[]>{
    const requestUrl = `${URL.baseUrl}${URL.usuariosByTarefaId_Get}`
    .replace(URL.usuariosByTarefaId_Get_Par1, '' + tarefaId);
