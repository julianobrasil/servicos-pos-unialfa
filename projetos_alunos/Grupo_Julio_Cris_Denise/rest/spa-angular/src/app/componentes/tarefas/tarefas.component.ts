...
export class TarefasComponent implements AfterViewInit {
  ...
  @Output() tarefaSelecionadaChanged = new EventEmitter<TarefaSelecionada>();

  constructor(private _tarefaService: TarefaService, private _alocacaoTarefaService: AlocacaoDeTarefasService, _fb: FormBuilder) {
  ...
  _seleciona(tarefa: TarefaEntityTo){
    let usuarios: Observable<UsuarioEntityTo[]> = this._alocacaoTarefaService.findAllUsersFromTarefaId(tarefa.id);
    let tarefaSelecionada : TarefaSelecionada = new TarefaSelecionada();
    tarefaSelecionada.tarefa = tarefa;
    usuarios.subscribe((data) => {
      tarefaSelecionada.usuarios = data ;
    });
   
    this.tarefaSelecionadaChanged.emit(tarefaSelecionada);
